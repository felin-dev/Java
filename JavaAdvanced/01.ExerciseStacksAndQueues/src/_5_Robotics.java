import java.util.ArrayDeque;
import java.util.LinkedHashMap;
import java.util.Map;
import java.util.Scanner;

public class _5_Robotics {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);

        String[] robotsInfo = sc.nextLine().split(";");
        LinkedHashMap<String, Integer> robots = getLinkedHashMap(robotsInfo);
        String time = sc.nextLine();
        int hours = Integer.parseInt(time.split(":")[0]);
        int minutes = Integer.parseInt(time.split(":")[1]);
        int seconds = Integer.parseInt(time.split(":")[2]);
        long totalTimeInSeconds = hours * 3600 + minutes * 60 + seconds;
        ArrayDeque<String> products = new ArrayDeque<>();
        int[] workingTime = new int[robots.size()];

        String product = sc.nextLine();
        while (!product.equals("End")) {
            products.offer(product);
            product = sc.nextLine();
        }

        while (!products.isEmpty()) {
            String currentProduct = products.poll();
            totalTimeInSeconds++;
            for (int robot = 0; robot < workingTime.length; robot++) {
                if (workingTime[robot] > 0) {
                    --workingTime[robot];
                }
            }
            boolean isAssigned = false;
            for (int i = 0; i < workingTime.length; i++) {
                if (workingTime[i] == 0) {
                    int count = 0;
                    for (Map.Entry<String, Integer> robot : robots.entrySet()) {
                        if (count == i) {
                            workingTime[i] = robot.getValue();
                            long takenHours = totalTimeInSeconds / 3600 % 24;
                            long takenMinutes = totalTimeInSeconds % 3600 / 60;
                            long takenSeconds = totalTimeInSeconds % 60;
                            System.out.printf("%s - %s [%02d:%02d:%02d]%n", robot.getKey(), currentProduct,
                                    takenHours, takenMinutes, takenSeconds);
                            isAssigned = true;
                            break;
                        }
                        count++;
                    }

                    break;
                }
            }
            if (!isAssigned) {
                products.offer(currentProduct);
            }
        }

        System.out.println();
    }

    private static LinkedHashMap<String, Integer> getLinkedHashMap(String[] robotsInfo) {
        LinkedHashMap<String, Integer> robots = new LinkedHashMap<>();
        for (String robot : robotsInfo) {
            String[] robotData = robot.split("-");
            String robotName = robotData[0];
            int robotSpeed = Integer.parseInt(robotData[1]);
            robots.put(robotName, robotSpeed);
        }
        
        return robots;
    }
}